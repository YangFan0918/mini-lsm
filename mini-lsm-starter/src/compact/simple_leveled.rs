...
impl SimpleLeveledCompactionController {
    ...
    pub fn apply_compaction_result(
        ...
        let mut file_need_to_move = Vec::new();
        if let Some(upper_level) = _task.upper_level {
            file_need_to_move.extend(&_task.upper_level_sst_ids);
            snapshot.levels[upper_level - 1].1.clear();
        } else {
            file_need_to_move.extend(&_task.upper_level_sst_ids);
            let len = _task.upper_level_sst_ids.len();
            for _ in 0..len {
                snapshot.l0_sstables.pop();
            }
        }
        file_need_to_move.extend(&_task.lower_level_sst_ids);
        snapshot.levels[_task.lower_level - 1].1 = _output.to_vec();
        (snapshot, file_need_to_move)
    }
}
